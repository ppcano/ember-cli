...
class Project {
  ...
  */
  ...
  blueprintLookupPaths() {
    let octaneBlueprintPath;
    if (this.isEmberCLIProject()) {
      octaneBlueprintPath = path.join(this.root, 'node_modules', 'ember-octane-blueprint-tmp', 'blueprints');
      let lookupPaths = [this.localBlueprintLookupPath(), octaneBlueprintPath];
      let addonLookupPaths = this.addonBlueprintLookupPaths();

      return lookupPaths.concat(addonLookupPaths);
    } else {
      octaneBlueprintPath = path.join(__dirname, '..', '..', 'node_modules', 'ember-octane-blueprint-tmp', 'blueprints');
      return this.addonBlueprintLookupPaths().concat([octaneBlueprintPath]);
    }
  }
